...
connection.onstream = function(event) {
  // don't display local video; this is mostly because the pi can't handle it
  // and anyway we want see it
  if(event.type!="local"){
   try{
    ...
    var options = {};
    // look for speech events - for the 'mouth' (led matrix)
    var speechEvents = hark(event.stream, options);
    console.log("speechEvents",speechEvents);
    speechEvents.on('speaking', function() {
      move_socket.send("speaking");
    });

    speechEvents.on('stopped_speaking', function() {
      console.log('stopped_speaking');
      move_socket.send("stopspeaking");
    });
